...
var apiWare=function(pDB)
{
    ...
    this.apify=function(app){
        app.apiRoute=function(options){
            ...
            if (options.cache){
                cacheDelItem=function(req,res,next){
                    if (options.cache&&options.cache.isAvailable){
                        var cacheKey="";
                        if (req.tenantId){cacheKey=options.routeName + "_" + req.tenantId + "_" + req.params.id;}
                        else{cacheKey=options.routeName + "_" + req.params.id;}
                ...
                cacheGetItem=function(req,res,next){
                    if (options.cache&&options.cache.isAvailable){
                        ...
                        options.cache.get(cacheKey,cacheItemRetrieved);
                    }else{
                        return next();
                    }
                }
                ...
                cacheSetItem=function(req,res,next){
                    if (options.cache&&options.cache.isAvailable) {
                        ...
                        if (req.kwaaires.status >= 200 && req.kwaaires.status < 299) {
                            options.cache.set(cacheKey,req.kwaaires.data);
                            return next();
                        } else {
                            return next();
                        }
            ...
            this.patch(route+"/:id",[].concat(options.preMW).concat(cacheDelItem).concat(options.preUpdateMW).concat(kwaaiCrudware.updatePart).concat(options.postUpdateMW).concat(options.postMW));
