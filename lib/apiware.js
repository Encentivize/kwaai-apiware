    ...
    apify:
        function(app){
            app.apiRoute=function(options){
                ...
                options.preMW.push(
                    kwaaiCrudware.setKwaaiOptions({collection: options.collection, schema: options.schema,validate:false,coerce:false,sendresponse:false})
                );

                if (options.roles){
                    options.preMW.push(kwaaiCrudware.onlyForRoles(options.roles));
                }
                ...
                if (options.useName){
                    ...
                    options.preCreateMW=options.preCreateMW.concat([
                        ...
                        ,function(req,res,next){
                            if (req.kwaaires.data.count>0){return res.send(409,"message template exists")}
                            next();
                        }
                    ]);
                }
                ...
                this.put(route+"/:id",[].concat(options.preMW).concat(cacheDelItem).concat([kwaaiCrudware.validateData,kwaaiCrudware.coerceData]).concat(options.preUpdateMW).concat(kwaaiCrudware.updateFull).concat(options.postUpdateMW).concat(options.postMW));
                this.patch(route+"/:id",[].concat(options.preMW).concat(cacheDelItem).concat([kwaaiCrudware.validateData,kwaaiCrudware.coerceData]).concat(options.preUpdateMW).concat(kwaaiCrudware.updatePart).concat(options.postUpdateMW).concat(options.postMW));
